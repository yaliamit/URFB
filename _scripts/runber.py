...
if 'loc' in outname:
    ...
    try:
        ...
        print('GIT failed')
    try:
        commands.check_output('ssh amit@marx.uchicago.edu "ssh amit@bernie.uchicago.edu \'cd /ga/amit/Desktop/Dropbox/Python; git pull\' " ',shell=True)
    except:
        print('pull failed')



    os.system('ssh amit@marx.uchicago.edu "ssh amit@bernie.uchicago.edu \'cd /ga/amit/Desktop/Dropbox/Python/TF/; ./runthingsBR.txt & \' & " & ')

    ny='no'
    while (ny != ''):
        time.sleep(10)
        try:
            ss=commands.check_output('scp amit@marx.uchicago.edu:/Volumes/amit/Desktop/Dropbox/Python/TF/'+OUTNAME+' '+ lOUTNAME,shell=True)
            print(ss)
            ny=''
        except:
            ny='no'
    ...
    while (not done):
        ...
        try:
            commands.check_output('scp amit@marx.uchicago.edu:/Volumes/amit/Desktop/Dropbox/Python/TF/'+OUTNAME+' ' + lOUTNAME,shell=True)
        except:
            print('copy failed')
    time.sleep(5)
